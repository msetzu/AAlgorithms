...
\begin{algorithmic}[1]
  \Function{query}{$I$, $i$, $sum$}:
    ...
    \State \If{lazy(I), $i \in I.left, i \neg \in I.right$}     \Comment{Lazy on left child}
        \State lazy(I) = \False\;
        \State query($I.left$, $i$, $sum + I.sum$)
    \EndIf
    
    \State \If{lazy(I), $i \in right_child, i \neg \in I.left$} \Comment{Lazy on right child}
        \State lazy(I) = \False\;
        \State query($I.right$, $i$, $sum + I.sum$)
    \EndIf

    \State \If{lazy(I), $i \in right_child, i \in I.left$}      \Comment{Lazy on both}
        \State lazy(I) = \False\;
    ...
    \State \If{!lazy(I), $i \in I.right, i \in I.left$}         \Comment{Not lazy on both}
